...
def get_dataloaders(df, fold, data_type, batch_size=4, num_workers=0, preprocessing = "uncropped"):
    ...
    Args:
        ...
        preprocessing (str):
            Type of preprocessing to conduct on images. "precropped" option crops the channels using mask, "uncropped" does not and instead includes mask as the 4th channel
...
def train_model(
    ...
    num_epochs: int = 10,
    n_folds: int = 5,
    preprocessing_type: str = "uncropped",
    ...
    Args:
        ...
        use_diff_lr (bool):
            If true will use a different learning rate for CNN and MLP components. Only for multimodal.
        lr (float):
        ...
        if use_diff_lr:
            optimizer = torch.optim.Adam([
                {"params": model.mri_encoder.parameters(),
                 "lr": cnn_lr,},
                {"params": list(model.clin_encoder.parameters()) + list(model.risk_predictor.parameters()),
                 "lr": mlp_lr,},
            ],
                weight_decay=1e-4,
            )          
